...
module "grafana_policy" {
  source = "git::https://git.tools.tsys.aws/scm/cdm/gpn-aws-terraform-modules.git//src/iam/iam-policy?ref=main"
...
module "grafana_irsa_role" {
  source     = "git::https://git.tools.tsys.aws/scm/cdm/gpn-aws-terraform-modules.git//src/iam/iam-assumable-role-with-oidc?ref=main"
  product_name = "${local.product_name}-${local.business_unit}"
  cluster_name = local.cluster_name
  purpose      = local.purpose
...
module "commontags" {
  source = "git::https://git.tools.tsys.aws/scm/cdm/gpn-aws-terraform-modules.git//src/commontags?ref=v5.0.0"
